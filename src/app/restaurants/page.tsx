...
export const metadata: Metadata = {
  ...
  description:
    "Gemfield plans for restaurants, bars and taphouses: Front Door $197/mo, Foundation $497/mo, Growth $997/mo, Scale from $1,497/mo. A menu you control from your phone, bookings that arrive during service, and ordering you keep the margin on.",
};
...
function buildJsonLd(promoLive: boolean) {
  const faq = promoLive ? [...R.faq.items, R.faq.promoItem] : R.faq.items;
  return {
    "@context": "https://schema.org",
    "@graph": [
      {
        "@type": "Service",
        "@id": `${SITE.url}/restaurants#service`,
...
export default function RestaurantsPage() {
  ...
  const faqItems = promoLive ? [...R.faq.items, R.faq.promoItem] : R.faq.items;
  ...
  return (
    <>
      ...
      {/* 2 — OCTOBER PROMOTION. Full width, directly under the hero.
          Gated on the promo window: this block, the Front Door paragraph and
          the FAQ entry all come down together on 1 November. */}
      {promoLive && (
        <section className="band blueprint">
          <div className="container-g on-dark-focus grid items-center gap-8 py-12 lg:grid-cols-12 lg:py-14">
            <Reveal className="lg:col-span-8">
              <p className="eyebrow !text-[#7fc8ad]">{R.promo.label}</p>
              <h2 className="font-display mt-3 text-[clamp(1.5rem,2.6vw,2.1rem)]">{R.promo.h}</h2>
              <p className="mt-4 max-w-2xl text-[0.98rem] leading-relaxed text-band-mut">
                {R.promo.body.map((seg, i) =>
                  seg.em ? (
            ...
            <Reveal className="lg:col-span-4 lg:justify-self-end" delay={100}>
              <a
                href={SITE.calendly}
                target="_blank"
                rel="noopener noreferrer"
                className="btn btn-on-dark !px-6 !py-3.5"
              >
                {R.promo.cta}
                <ArrowRight size={15} />
              </a>
                  ...
                  <tr className="border-b border-hairline align-bottom">
                    ...
                    {R.table.tiers.map((t) => {
                      ...
                      return (
                        ...
                        >
                          {flagged && (
                            <span className="mono-num mb-2 inline-block rounded-full bg-emerald px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.1em] text-white">
                              {R.table.promoBadge}
                            </span>
                          )}
            ...
            {R.table.tiers.map((t, ti) => {
              ...
              return (
                <Reveal key={t.name} delay={Math.min(ti * 60, 180)}>
                  ...
                  >
                    {flagged && (
                      <span className="mono-num mb-3 self-start rounded-full bg-tint px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.1em] text-emerald-deep">
                        {R.table.promoBadge}
                      </span>
                    )}
